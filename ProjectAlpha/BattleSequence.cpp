...
void BattleSequence::buildEnemies()
{
	...
	while (counter < size)
	{
		...
		curr_enemies[counter] = EnemyActor(enemy_list[pod.set_id[counter]].getEName(),
			...
			enemy_list[pod.set_id[counter]].gold);
...
//Builds a quick GUI of enemies. Alert player if a target is untargetable. Nothing fancy to remove dead enemies YET
...
int BattleSequence::physicalSpellcast(int spell_id)
{
	...
	if (!(spell_list[spell_id].aoe))				//For single target spells
	{
		...
		if (enemy_target < 0)						//Player wants to return. Output a negative damage value and feed it back to cast spell
		{
			damage_roll = -1;
		}
		else
	...
	else				//AOE spells. Deal damage to each monster in list with a variance for each one
	{
		std::cout << spell_list[spell_id].name << " dealt\n";
		while (counter < pod.set_size)
		{
			if (curr_enemies[counter].health > 0)				//Check if monster is alive
			{
				variance = damageVariance();
				damage_roll = ((player->c_stats.strength + (player->c_stats.proficiency / 2)) * spell_list[spell_id].modifier) + spell_list[spell_id].base_damage;
				damage_roll = damage_roll * variance;				//Add variance in damage
...
int BattleSequence::magicalSpellcast(int spell_id)
{
	...
	if (!spell_list[spell_id].aoe)
	{
		...
		if (enemy_target < 0)				//Case if player wants to return
		{
			damage_roll = -1;
		}
		else
